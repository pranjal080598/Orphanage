             ...
             if(mysqli_num_rows($result) > 0){
                // Outer While
                 while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                        ...
                        $sql1= "SELECT * FROM orphanation.orders WHERE orphanage_id='{$orphanage_id}'";
                        $result1 = $conn ->query($sql1);
                        // $row1= $result1 ->fetch_assoc();
                        //Inner While
                        while($row1 = $result1 ->fetch_assoc()){
                            $sql2= "SELECT * FROM orphanation.orphanage WHERE orphanage_id='{$orphanage_id}'";
                            // $result2 = $conn ->query($sql2);
                            $result2=mysqli_query($link,$sql2);
                            //$row2=$result2->fetch_row();
                            $row2=mysqli_fetch_row($result2);
                            echo '<span style="background-color:#AFA;text-align:center;">';
                            echo "<p>" . $row1["orphanage_id"]." - ".$row2['2']  . "</p>";
                            break;
                        }//inner while(2) 
                }//outer While(1) 
            }//End Of if
